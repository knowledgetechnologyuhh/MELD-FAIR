...
def train_er_model(result_folder_path, batch_size = 64, initial_lr = 1e-4, num_workers = 4, video = True, audio = True, frame_sequence_length = 15, device_index = 0):
    splits = ["train", "test", "dev"]
    classes = ["anger", "disgust", "fear", "joy", "neutral", "sadness", "surprise"]
    num_classes = len(classes)
    
    data_collation_device = "cpu" # Using cpu as the device during the data collation
    if audio:
        spec_transform, melscale_transform = audio_processor.get_spec_and_melscale_transforms(data_collation_device)
    else:
        spec_transform, melscale_transform = None, None
    
    train_data = MELDDataset(classes, "train")
    test_data = MELDDataset(classes, "test")
    dev_data = MELDDataset(classes, "dev")
    
    train_loader = DataLoader(train_data, batch_size = batch_size, shuffle = True, num_workers = num_workers, collate_fn = lambda data : collate_data(data, "train", num_classes, data_collation_device, video, audio, frame_sequence_length, spec_transform, melscale_transform), pin_memory = True)
    test_loader = DataLoader(test_data, batch_size = batch_size, shuffle = False, num_workers = num_workers, collate_fn = lambda data : collate_data(data, "test", num_classes, data_collation_device, video, audio, frame_sequence_length, spec_transform, melscale_transform), pin_memory = True)
    dev_loader = DataLoader(dev_data, batch_size = batch_size, shuffle = False, num_workers = num_workers, collate_fn = lambda data : collate_data(data, "dev", num_classes, data_collation_device, video, audio, frame_sequence_length, spec_transform, melscale_transform), pin_memory = True)
    
    softmax_layer = nn.Softmax(dim = 1)
    
    device = "cuda:{:d}".format(device_index) if torch.cuda.is_available() and device_index >= 0 else "cpu"
    ...
    while len(history["f1"]["dev"]) < 2 or epoch < history["f1"]["dev"].index(best["f1"]) + max_add_epochs or len(history["f1"]["dev"]) < max_add_epochs:
        ...
        for spl in splits:        
            ...
            if result_folder_path is not None and result_log is not None:
                ...
                fout.write("\tEpoch {:d} - {} started at {:%Y-%m-%d %H:%M:%S}".format(epoch + 1, "Training" if spl == "train" else "Evaluation", epochstart) + os.sep)
                fout.close()
            ...
            if result_folder_path is not None and result_log is not None:
                fout = open(os.path.join(result_folder_path, result_log), "a")
                fout.write("{:%Y-%m-%d %H:%M:%S} {:^5} - [Epoch {:d}/{:d}] Loss: {:.6f} Acc: {:.6f} Bal Acc: {:.6f} F1: {:.6f}".format(datetime.now(), spl.upper(),
                                                                                                                                       epoch + 1, total_epochs,
                                                                                                                                       history["loss"][spl][-1],
                                                                                                                                       history["acc"][spl][-1],
                                                                                                                                       history["bal_acc"][spl][-1],
                                                                                                                                       history["f1"][spl][-1]) + os.sep)
                fout.write(str(cm) + os.sep + os.sep + ("" if spl == "train" else os.sep))
                fout.close()
...
if __name__ == "__main__":
    ...
    train_er_model(args.result_folder_path, args.batch_size, args.lr, args.num_workers, args.video, args.audio, args.face_sequence_length, args.cuda_device_index)
