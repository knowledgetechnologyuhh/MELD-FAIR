...
def extract_MFCC(waveform, spec_transform, melscale_transform, device, amplitude_scale = 2 ** 30, mfcc_power_scale = 1):
    waveform /= torch.max(waveform)
    waveform *= amplitude_scale
    preemphasis_coefficient = 0.97
    audio = torch.cat((waveform[0].unsqueeze(0), waveform[1 :] - preemphasis_coefficient * waveform[: -1]))
    spec = spec_transform(waveform)
    energy = torch.sum(spec, axis = 0)
    melspec = melscale_transform(spec)
    log_offset = 1e-6
    melspec = torch.log(melspec + log_offset)
    dct_mat = TAF.create_dct(cfg.n_mfcc, melscale_transform.n_mels, "ortho").to(device)
    mfcc = torch.matmul(melspec.transpose(-1, -2), dct_mat).transpose(-1, -2)
    mfcc[0] = torch.log(energy + log_offset) * mfcc_power_scale
